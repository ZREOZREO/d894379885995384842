...
module.exports = {
    ...
    run: async (bot, message, args, ops) => {
        ...
        if (!channel) return message.channel.send('노래를 재생하기 위해서는 봇과 같은 음성채널에 있어야합니다');

        if (!args[0]) return message.channel.send("재생하고 싶은 영상의 제목이나 링크를 입력해주세요")
        ...
        if (!permissions.has('CONNECT')) return message.channel.send('접속할 수 있는 권한이 없습니다');
        if (!permissions.has('SPEAK')) return message.channel.send('노래를 재생할 수 있는 권한이 없습니단');
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            ...
            return message.channel.send(`**Playlist \`${playlist.title}\`가 재생목록에 추가되었습니다**`);
        } else {
            try {
                ...
                try {
                    ...
                    console.error(err)
                    return message.channel.send(`노래를 찾을 수 없습니다`)
                }
            }
        ...
        async function handleVideo(video, message, channel, playlist = false) {
            ...
            if (serverQueue) {
                ...
                if (playlist) return undefined;
                    ...
                    const sembed = new MessageEmbed()
                        .setColor("#FFFFFF")
                        .setTitle("재생목록에 추가", message.author.displayAvatarURL())
            ...
            try {
                ...
                console.error(`음성채널에 접속할 수 없습니다: ${error.message}`);
                ops.queue.delete(message.guild.id);
                await channel.leave();
                return message.channel.send(`음성채널에 접속할 수 없습니다: ${error.message}`);
            }
        };
        async function play(song) {
            ...
            const embed = new MessageEmbed()
                ...
                .setFooter(`${message.member.displayName}에 의해 재생됨 `, message.author.displayAvatarURL());
            ...
            collector.on('collect', async (b) => {
                ...
                if (b.id == "pause") {
                    if (!channel) return await b.reply.send('음성채널에 접속해야합니다', true);
                    if (message.guild.me.voice.channel !== message.member.voice.channel) {
                        return await b.reply.send('음성채널에 접속해야합니다', true);
                    }
                    ...
                    if (serverQueue && serverQueue.playing) {
                        ...
                        return await b.reply.send('노래가 일시정지 됨', true);

                    }

                    return await b.reply.send('재생되고 있는 것이 없거나 이미 일시정지 되었습니다', true);
                }
                if (b.id == "resume") {
                    ...
                    return await b.reply.send('재생되고 있는 것이 없거나 이미 일시정지가 해제되었습니다', true);
                }
                ...
                if (b.id == "skip") {
                    ...
                    if (b.clicker.id === message.author.id) {
                        ...
                        return await b.reply.send('건너뛰기 됨', true)
                    }
                    return await b.reply.send(`오직 ${message.author.username}만이 스킵 할 수 있습니다`, true)
                }
                if (b.id == "loop") {
                    if (!channel) return await b.reply.send('음성채널에 접속해있지 않습니다', true);

                    if (b.clicker.id === message.author.id) {
                        if (!serverQueue) return await b.reply.send('재생되고 있는 것이 없습니다', true);
                        if (message.guild.me.voice.channel !== message.member.voice.channel) {
                            return await b.reply.send('You have to join my VC', true);
                        }
                        if (!serverQueue.loop) {
                            serverQueue.loop = true;
                            return await b.reply.send('반복 활성화', true);
                        } else {
                            serverQueue.loop = false;
                            return await b.reply.send('반복 비활성화', true);
                        }
                    }
                    return await b.reply.send(`오직 ${message.author.username}만이 반복을 활성화 할 수 있습니다`, true)
                }
            });
